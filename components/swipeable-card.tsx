...
export function SwipeableCard({
  ...
  useEffect(() => {
    ...
    const handleEnd = () => {
      ...
      if (Math.abs(dragOffset.x) > SWIPE_THRESHOLD) {
        ...
        setDragOffset({ x: direction === "right" ? 1000 : -1000, y: dragOffset.y })
        setTimeout(() => {
          onSwipe(direction)
  ...
  const handleLike = () => isInteractive && onSwipe("right")
  const handleDislike = () => isInteractive && onSwipe("left")
      ...
      {isInteractive && (
        <>
          <div className="absolute bottom-40 left-0 right-0 flex items-center justify-center gap-6 p-4">
            <Button size="lg" variant="outline" onClick={handleDislike} className="h-16 w-16 rounded-full shadow-lg bg-white text-gray-900">
              <X className="h-8 w-8" />
            </Button>
            <Button size="lg" variant="default" onClick={handleLike} className="h-16 w-16 rounded-full shadow-lg bg-red-500 text-white">
              <Heart className="h-8 w-8" />
            </Button>
          </div>
          <div className="absolute bottom-24 right-6">
            <Button size="sm" variant="secondary" onClick={onComment} className="h-10 w-10 rounded-full shadow-lg">
              <MessageCircle className="h-4 w-4" />
            </Button>
          </div>
        </>
